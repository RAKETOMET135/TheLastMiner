export class StringConverter{
    static floatFromPixelString(pixelString){
        return parseFloat(pixelString.slice(0, pixelString.length - 2))
    }

    static floatFromDegString(degString){
        return parseFloat(degString.slice(0, degString.length - 3))
    }

    static abbreviateNumber(number, symbol){
